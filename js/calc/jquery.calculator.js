/* http://keith-wood.name/calculator.html
   ...
   Modifyed calculates with bigdecimal library, by Mikage Sawatari (mikage at mikage.to) 2008/11/03
 */
...
function Calculator() {
	...
	this._keyDefs = {
		...
		'EX': ['e竅ｿ', this.unary, '._exp', 'exp', 'EXP', 'E'],
		'SQ': ['x2', this.unary, '._sqr', 'sqr', 'SQR', '@'],
...
$.extend(Calculator.prototype, {
	...
	_add: function(inst) {
		inst.curValue = (new BigDecimal(String(inst.prevValue))).add(new BigDecimal(String(inst.curValue))).toString();
	},

	_subtract: function(inst) {
		inst.curValue = (new BigDecimal(String(inst.prevValue))).subtract(new BigDecimal(String(inst.curValue))).toString();
	},

	_multiply: function(inst) {
		inst.curValue = (new BigDecimal(String(inst.prevValue))).multiply(new BigDecimal(String(inst.curValue))).toString();
	},

	_divide: function(inst) {
		inst.curValue = (new BigDecimal(String(inst.prevValue))).divide(new BigDecimal(String(inst.curValue))).toString();
	},
